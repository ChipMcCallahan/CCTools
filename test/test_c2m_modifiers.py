...
class TestC2MModifiers(unittest.TestCase):
    ...
    def test_wire_modifier_parse(self):
        ...
        result = C2MModifiers.parse(tile_id, test_value)
    ...
    def test_wire_modifier_build(self):
        ...
        # combined => 0x16
        result_bytes = C2MModifiers.build(tile_id, data)
    ...
    def test_wire_modifier_invalid_length(self):
        ...
        with self.assertRaises(ValueError):
            C2MModifiers.parse(tile_id, bytes([0x01, 0x02]))  # 2 bytes is invalid
    ...
    def test_letter_tile_parse(self):
        ...
        # 0x1E => "↓" arrow
        result_arrow = C2MModifiers.parse(tile_id, bytes([0x1E]))
        self.assertEqual(result_arrow["char"], "↓")

        # 0x41 => 'A'
        result_char = C2MModifiers.parse(tile_id, bytes([0x41]))
        self.assertEqual(result_char["char"], "A")

        # 0x1B => outside arrow range, not ASCII 20..5F => None
        result_none = C2MModifiers.parse(tile_id, bytes([0x1B]))
    ...
    def test_letter_tile_build(self):
        ...
        # Arrow
        data_arrow = {"char": "←"}
        result_arrow = C2MModifiers.build(tile_id, data_arrow)
        self.assertEqual(result_arrow, bytes([0x1F]))

        # ASCII
        data_char = {"char": "Z"}
        result_char = C2MModifiers.build(tile_id, data_char)
        self.assertEqual(result_char, bytes([0x5A]))  # 'Z' => 0x5A

        # None or invalid => 0
        data_invalid = {"char": "ß"}  # outside 0x20..0x5F
        result_invalid = C2MModifiers.build(tile_id, data_invalid)
    ...
    def test_clone_machine_parse(self):
        ...
        result = C2MModifiers.parse(tile_id, bytes([0x0D]))
    ...
    def test_custom_tiles_parse(self):
        ...
        result = C2MModifiers.parse(tile_id, bytes([2]))
        self.assertEqual(result["style"], "Yellow")

        # Invalid style
        with self.assertRaises(ValueError):
            C2MModifiers.parse(tile_id, bytes([9]))
    ...
    def test_custom_tiles_build(self):
        ...
        tile_id = CC2.CUSTOM_FLOOR
        data = {"style": "Blue"}
        result = C2MModifiers.build(tile_id, data)
        self.assertEqual(result, bytes([3]))  # 'Blue' => 3

        # Invalid style
        data_invalid = {"style": "Rainbow"}
        with self.assertRaises(ValueError):
            C2MModifiers.build(tile_id, data_invalid)
    ...
    def test_logic_gate_parse_inverter(self):
        ...
        result = C2MModifiers.parse(tile_id, bytes([0x01]))
    ...
    def test_logic_gate_parse_counter(self):
        ...
        """
        tile_id = CC2.LOGIC_GATE
        result = C2MModifiers.parse(tile_id, bytes([0x23]))
    ...
    def test_logic_gate_build_invalid_direction(self):
        ...
        tile_id = CC2.LOGIC_GATE
        with self.assertRaises(ValueError):
            C2MModifiers.build(tile_id, {"gate": "XOR_X"})
    ...
    def test_railroad_track_parse(self):
        ...
        result = C2MModifiers.parse(tile_id, track_bytes)
    ...
    def test_railroad_track_invalid_length(self):
        ...
        tile_id = CC2.RAILROAD_TRACK
        with self.assertRaises(ValueError):
            C2MModifiers.parse(tile_id, bytes([0x01]))  # only 1 byte
    ...
    def test_cannot_apply_modifier_to_unknown_tile(self):
        ...
        """
        with self.assertRaises(ValueError):
            C2MModifiers.parse(CC2.WATER, bytes([0x00]))  # Tile that does not accept modifiers.

        with self.assertRaises(ValueError):
            C2MModifiers.build(CC2.WATER, {})  # same for build
